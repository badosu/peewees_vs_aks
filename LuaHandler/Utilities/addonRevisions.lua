...
function AddonRevs.NewAddonRev2()
	...
	tmergein(addonEnv.addon, {
		Remove = function()
			handler:Remove(addon, "auto")
		end,
		IsMouseOwner = function()
		...
		UpdateCallIn = function(name)
			handler:UpdateAddonCallIn(name, addon)
		end,
		RemoveCallIn = function(name)
			handler:RemoveAddonCallIn(name, addon)
		end,
		--[[
FIXME???
...
--]]
		RegisterGlobal = function(name, value)
			return handler:RegisterGlobal(addon, name, value)
		end,
		DeregisterGlobal = function(name)
			return handler:DeregisterGlobal(addon, name)
		end,
		SetGlobal = function(name, value)
			return handler:SetGlobal(addon, name, value)
		end,
	})
...
function AddonRevs.NewAddonRev1()
	...
	addonEnv.include = function(f)
		return include(f, addon)
	end
	h.ForceLayout = handler.EG.ForceLayout
	h.RemoveWidget = function()
		handler:Remove(addon, "auto")
	end
	h.GetCommands = function()
	...
	h.UpdateCallIn = function(_, name)
		handler:UpdateAddonCallIn(name, addon)
	end
	h.RemoveCallIn = function(_, name)
		handler:RemoveAddonCallIn(name, addon)
	end
	...
	h.RegisterGlobal = function(_, name, value)
		return handler:RegisterGlobal(addon, name, value)
	end
	h.DeregisterGlobal = function(_, name)
		return handler:DeregisterGlobal(addon, name)
	end
	h.SetGlobal = function(_, name, value)
		return handler:SetGlobal(addon, name, value)
	end
