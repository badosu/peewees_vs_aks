...
function tprint(tbl, indent)
	...
	for k, v in pairs(tbl) do
		...
		elseif kind == "userdata" then
			Spring.Echo(formatting .. "<userdata>")
		elseif kind == "function" then
			Spring.Echo(formatting .. "<function>")
		elseif kind == "boolean" then
			Spring.Echo(formatting .. (v and "true" or "false"))
		else
...
handler = {
	...
	knownCallIns = {},
	callInLists = setmetatable({}, {
		__index = function(self, key)
			self[key] = CreateList(key, SortAddonsFunc)
			return self[key]
		end,
	}),
	callInHookFuncs = {},
...
local function s(str)
	return str:gsub("%%{addon}", handler.addonName):gsub("%%{Addon}", handler.AddonName)
end
...
handler:AddNewCallIn("AddonWillBeRemoved", true, false) --// (addon, reason) -- reason can either be "crash" | "user" | "auto" | "dependency"
		...
		if order > 0 then
			...
			if ki then
				...
				if handler.verbose then
					Spring.Log(LUA_NAME, "warning", ('Couldn\'t find a %s named "%s"'):format(handler.addonName, name))
				end
...
function handler:DetectEnabledAddons()
	for _, ki in pairs(handler.knownInfos) do
		if not ki.active then
			...
			if ((order or 0) > 0) or ((order == nil) and defEnabled) then
...
local function RemoveAddonCallIns(addon)
	...
	for ciName, ciList in pairs(handler.callInLists) do
		assert(
			type(ciList) == "table",
			"[RemoveAddonCallIns] pre-removing callin " .. ciName .. " from addon " .. addon._info.name
		)
		handler:RemoveAddonCallIn(ciName, addon)
		assert(
			type(ciList) == "table",
			"[RemoveAddonCallIns] post-removing callin " .. ciName .. " from addon " .. addon._info.name
		)
	end
...
function handler:Load(filepath, _VFSMODE)
	...
	local mt = {
		__index = ki,
		__newindex = function()
			error("_info tables are read-only", 2)
		end,
	...
	if handler.verbose or handler.initialized then
		...
		Spring.Log(
			LUA_NAME,
			"info",
			("%-20s %-21s  %s"):format(loadingstr, name, handler:GetFancyString(name, basename))
		)
	end
	...
	--// Link the CallIns
	for ciName, _ in knownCallins(addon) do
...
function handler:Remove(addon, _reason)
	...
	for i = 1, #rem do
		...
		Spring.Log(
			LUA_NAME,
			"info",
			('Removed %s:  %-21s  %s (dependent of "%s")'):format(
				handler.addonName,
				ki2.name,
				handler:GetFancyString(ki2.name, ki2.basename),
				name
			)
		)
...
local function LoadConfigFile(filename, envParam)
	...
	elseif type(rvalue) ~= "table" then
		Spring.Log(
			LUA_NAME,
			"warning",
			"Failed to load: " .. filename .. "  (table data expected, got " .. type(rvalue) .. ")"
		)
	else
		return rvalue
	end
...
function handler:LoadOrderList()
	if LUA_NAME ~= "LuaUI" and LUA_NAME ~= "LuaMenu" then
		return
	end
...
function handler:RegisterGlobal(owner, name, value)
	if (name == nil) or _G[name] or handler.globals[name] or engineCallIns[name] then
		return false
	end
	_G[name] = value
...
function handler:UpdateCallIn(ciName)
	...
	if
		ciList.first or staticCallInList[ciName]
		-- or ((ciName == "GotChatMsg") and actionHandler.HaveChatAction()) --FIXME these are LuaRules only
		-- or ((ciName == "RecvFromSynced") and actionHandler.HaveSyncAction()) --FIXME these are LuaRules only
	then
		--// already exists?
